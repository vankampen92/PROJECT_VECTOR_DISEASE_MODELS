...
void Human_Population_Random_Initial_Condition_Given_i__0( Parameter_Table * P, double * Y, 
							   double y )
{
  /* Random initial condition given that the fraction of infectious humans, y, is fixed */
  ...
  /* P->r__0 can be also randomly chosen from Boundary function when Boundary.c file
     is updated to include this possibility 
  */
#if defined CASES_1   
    P->r__0 = gsl_rng_uniform(r);
#endif 
#if defined NO_cASES
    P->r__0 = gsl_rng_uniform(r);
#endif 

  R_E_S_C_A_L_I_N_G___I_N_I_T_I_A_L___C_O_N_D_I_T_I_O_N_S___G_I_V_E_N___y( P, 
...
#if defined CASES_1   
    x = (P->s__0 + P->e__0 + P->r__0 + P->c__0 );
#endif
#if defined NO_CASES   
    x = (P->s__0 + P->e__0 + P->r__0 );
#endif
#if defined SEnI  
    x = (P->s__0 + P->e__0 );
#endif
