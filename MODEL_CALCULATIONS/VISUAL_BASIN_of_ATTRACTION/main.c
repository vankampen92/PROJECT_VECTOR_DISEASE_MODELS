...
/* This code calculates basins of attraction in case of bi-stability. 
   ...
   . ~$ ./ODE_RESTRICTED_JACOBIAN_SI-XW_RANDOM_INITIAL_CONDITION_$1 -R0 $2 -S0 19 -n 2 -v0 3 -v1 5 -v2 30 -G0 1 -G1 1 -G5 1 -G6 0.0 -G7 1.0 -t0 4 -t1 0.0 -t2 5000.0  -G8 1 -G9 8 -G10 1 -G11 7 -G12 1 -H1 1.5E-5 -H2 1.5E-5 -H3 6.7E-4 -H4 0.01 -H5 0 -H7 0.00005 -M0 0.33 -M1 0.1 -M2 0.05 -M3 0.05 -M9 0.6 -P0 0   -TP 12 -TY -1
*/
...
#include <gsl_random_number_Setup.c>
  ...
  No_of_POINTS_0 = 40;  No_of_POINTS_1 = 40;
  int No_of_POINTS_GRID = No_of_POINTS_0 * No_of_POINTS_1 ;
  double * W_GRID = (double *)calloc( No_of_POINTS_GRID, sizeof(double) );

  double *  X_0 = (double *)calloc( No_of_POINTS_GRID, sizeof(double) ); 
  double *  Y_0 = (double *)calloc( No_of_POINTS_GRID, sizeof(double) );  
  double *  X_1 = (double *)calloc( No_of_POINTS_GRID, sizeof(double) ); 
  double *  Y_1 = (double *)calloc( No_of_POINTS_GRID, sizeof(double) ); 
#endif
#if defined RANDOM_SCAN
  Type_of_Calculation = 1;
  No_of_POINTS_0 = Table.Realizations;  No_of_POINTS_1 = Table.Realizations;
  double * W_GRID = (double *)calloc( Table.Realizations, sizeof(double) );

  double *  X_0 = (double *)calloc( Table.Realizations, sizeof(double) ); 
  double *  Y_0 = (double *)calloc( Table.Realizations, sizeof(double) );  
  double *  X_1 = (double *)calloc( Table.Realizations, sizeof(double) ); 
  double *  Y_1 = (double *)calloc( Table.Realizations, sizeof(double) ); 
#endif
...
#if defined BASIN_SCAN
  printf (" Total Scanned Points: N = %d\n", (No_of_POINTS_0 * No_of_POINTS_1));
  if ( (No_of_POINTS_0 * No_of_POINTS_1) < (N_0 + N_1) )  {
  ...
  int FIRST_PLOT = 0;
  ...
  #if defined RANDOM_SCAN
    FIRST_PLOT = 0;
    Table.CPG->type_of_Symbol  = 7;
    Table.CPG->color_Index     = 3;  /* 14: Dark gray */
    ...
    /* B E G I N : Representing the upper, inter and lower steady-state stationary point */
    C_P_G___P_L_O_T_T_I_N_G___M_U_L_T_I_S_T_A_B_I_L_I_T_Y ( &Table ); 
    /*   E N D : ------------------------------------------------------------------------*/
#endif
  /*********************************************************************/      
