...
void stable_Solution(double y, double W, Parameter_Table *P, double *Y)
{
  ...
  */
  ...
  I = Y[I_] = P->H * B / (B + P->H_Recov + P->H_Delta);                            /* I   */
#if defined ODE  
  S = Y[S_]  = P->H * (P->H_Delta + P->H_Recov ) / (B + P->H_Recov + P->H_Delta);   /* S   */
#endif
#if defined ODE_RESTRICTED_JACOBIAN
  S = P->H * (P->H_Delta + P->H_Recov ) / (B + P->H_Recov + P->H_Delta);   /* S   */    
#endif
...
void stable_Solution_Free_Disease(Parameter_Table *P, double *Y)
{
  ...
  Y[I] = 0.0;                            /* I  Infectious class */
  
  /* Mosquito */
  M = Total_Mosquito_Steady_State(P);
  Stable_Solution_Partial_Mosquito_Larva_Free_Disease (P, M, L, W, Y );
